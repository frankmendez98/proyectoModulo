...
this.focusTarget2($target);
		...
		focusTarget2: function($target) {
			$target.find(this.options.focusTarget2).focus();
		},
		/**
		 * Bind main plugin events
		 * @return {void}
		...
		bindEvents: function() {
			...
			this.$table
				...
				.on('keydown.' + this.options.namespace, this.options.listenTarget2, $.proxy(moveEvent, this))
				.on('keyup.' + this.options.namespace, this.options.listenTarget2, keyup);
		},
